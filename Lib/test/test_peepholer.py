    ...
    def check_jump_targets(self, code):
        ...
        for instr in instructions:
            ...
            if tgt.opname in ("JUMP_BACKWARD", "JUMP_FORWARD"):
                self.fail(
                    f"{instr.opname} at {instr.offset} "
                    f"jumps to {tgt.opname} at {tgt.offset}"
                )
            # unconditional jump to RETURN_VALUE
            if (
                instr.opname in ("JUMP_BACKWARD", "JUMP_FORWARD")
                and tgt.opname == "RETURN_VALUE"
            ):
                self.fail(
                    f"{instr.opname} at {instr.offset} "
                    f"jumps to {tgt.opname} at {tgt.offset}"
...
class DirectCfgOptimizerTests(CfgOptimizationTestCase):
    ...
    def test_conditional_jump_forward_non_const_condition(self):
        ...
        self.cfg_optimization_test(
            insts, expected_insts, consts=[0, 1, 2, 3, 4], expected_consts=[0, 2, 3]
        )
    ...
    def test_conditional_jump_forward_const_condition(self):
        ...
        self.cfg_optimization_test(
            insts, expected_insts, consts=[0, 1, 2, 3, 4], expected_consts=[0, 3]
        )
