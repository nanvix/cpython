...
def check_sanitizer(*, address=False, memory=False, ub=False, thread=False):
    ...
    thread_sanitizer = (
        "-fsanitize=thread" in cflags or "--with-thread-sanitizer" in config_args
    )
    return (
        (memory and memory_sanitizer)
        or (address and address_sanitizer)
        or (ub and ub_sanitizer)
        or (thread and thread_sanitizer)
    )


def skip_if_sanitizer(
...
has_fork_support = (
    hasattr(os, "fork") and not is_emscripten and not is_wasi and not is_nanvix
)
...
def missing_compiler_executable(cmd_names=[]):
    ...
    from setuptools._distutils import ccompiler, spawn, sysconfig
