...
| test_clinic | Module-level `SkipTest` in `test_tools.__init__` | `has_subprocess_support` is `False` — entire module skipped at import (#328) |
| test_cppext | `@requires_subprocess()` on `TestCPPExt` | Both `test_build_cpp11` and `test_build_cpp03` auto-skip (#328) |
...
| test_capi | Crashes at import time — `test_capi.test_dict` and other sub-modules do `import _testcapi` at module level; `_testcapi` C extension is not built for Nanvix. Regrtest reports "errors while loading tests" and aborts. (#328) |
| test_ctypes | Crashes at import time — 17 sub-modules (`test_as_parameter`, `test_bitfields`, `test_callbacks`, `test_cfuncs`, `test_funcptr`, `test_functions`, `test_libc`, `test_pickling`, `test_pointers`, `test_prototypes`, `test_refcounts`, `test_returnfuncptrs`, `test_slicing`, `test_stringptr`, `test_structures`, `test_unicode`, `test_values`, `test_win32`) do `import _ctypes_test` at module level; `_ctypes_test` C extension is not built for Nanvix. Regrtest reports "errors while loading tests" and aborts. (#328) |
| test_stable_abi_ctypes | Crashes at import time — does `from _testcapi import get_feature_macros` at module level; `_testcapi` C extension is not built for Nanvix. (#328) |
