...
## String, Encoding, and Serialization Tests (#323)

### test_glob

| Change | Reason |
|--------|--------|
| `tearDown`: wrapped `shutil.rmtree` in `try/except OSError` | Nanvix does not implement `rmdir` (returns `ENOSYS`). Cleanup must not crash the test run. |
...
### Clean-Pass Modules (#323)

- test_string, test_string_literals, test_ucn, test_format, test_fstring
- test_unicodedata, test_codeccallbacks
...
- test_textwrap, test_difflib, test_fnmatch, test_shlex, test_re
