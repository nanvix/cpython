...
### Using the Nanvix SDK

All Nanvix target compilation, archives, stripping, and final links run in the
...
The SDK provides Clang 22, LLVM tools, target headers, libc/libm, startup
objects, and the linker script. nanvix-zutil mounts SDK-built dependencies at
`/mnt/buildroot`; it never copies them into the downloaded runtime sysroot.
Host GCC and Python may be used only by native build generators.
...
Windows uses the same immutable SDK for target builds and the matching Nanvix
0.20.0 Windows runtime for test execution.
...
> **Note:** `.nanvix/out/test` is the test install tree created by
> `./z build`. Run the build before using the commands below.
...
```
Python 3.12.3 (...) [Clang 22.1.8] on nanvix
...
Nanvix 0.20.0 additionally advertises some libc entry points whose runtime
operation is unsupported. FIFO/device-node tests are skipped because
`mkfifo()`/`mknod()` return `ENOTSUP`; supplementary/reserved-ID tests are
skipped because the corresponding identity database and getters are absent.
The SDK libc's real and complex transcendental results do not meet CPython's
strict ULP fixtures, so only those fixture methods are skipped.
...
| SDK support | Immutable Nanvix Clang/LLVM SDK for every target artifact |
| Configure wrapper | Wraps standard `./configure` with Nanvix cross-compilation settings |
| Linking | Final static interpreter links through the SDK Clang driver |
...
| **Standalone ramfs limits** | A shared ramfs image (~54 MB) is built once; batches run sequentially and some modules remain excluded for heap limits |
...
| Daily schedule | Runs at 13:00 UTC |
